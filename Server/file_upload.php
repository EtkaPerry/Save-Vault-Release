...
function handleProfilePhotoUpload($file, $userId) {
    ...
    // Defence in depth: only ever read a genuine HTTP upload, never an
    // arbitrary server path that might be smuggled in via tmp_name.
    if (!is_uploaded_file($file['tmp_name'])) {
        return ['success' => false, 'message' => 'Invalid upload'];
    }

    // Re-encode the image through GD rather than storing the exact bytes the
    // client sent. Only freshly-rendered pixel data is written to disk, which
    // strips any embedded PHP/script/polyglot payload or hostile metadata.
    if (!reencodeImageToFile($file['tmp_name'], $detectedType, $uploadPath)) {
...
/**
 * Re-encode an uploaded image through GD and write a clean copy to $destPath,
 * discarding everything except the decoded pixel data. This is the key defence
 * that neutralises image/PHP polyglots and malicious metadata, because the
 * stored file is generated by GD rather than copied from the client.
 *
 * @param string $srcPath  Path to the validated temporary upload.
 * @param string $mimeType Detected MIME type (image/jpeg|png|gif).
 * @param string $destPath Destination path to write the sanitised image to.
 * @return bool   true on success.
 */
function reencodeImageToFile($srcPath, $mimeType, $destPath) {
    switch ($mimeType) {
        case 'image/jpeg':
    ...
    try {
        switch ($mimeType) {
            case 'image/jpeg':
                ...
                break;
            case 'image/png':
                // Preserve transparency when re-encoding PNGs.
                imagealphablending($image, false);
                imagesavealpha($image, true);
                $ok = imagepng($image, $destPath);
                break;
            case 'image/gif':
                $ok = imagegif($image, $destPath);
                break;
            default:
                $ok = false;
        }
